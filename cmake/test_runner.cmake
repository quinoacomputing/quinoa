...
# Run the test
set(test_command ${CHARMRUN} +p${NUMPES} -bind-to none ${TEST_EXECUTABLE}
                 ${TEST_EXECUTABLE_ARGS})
string(REPLACE ";" " " test_command_string "${test_command}")
message("\nRunning test command: '${test_command_string}'\n")
execute_process(COMMAND ${test_command} RESULT_VARIABLE ERRCODE)

# Check return value from test
if(ERRCODE)

  message(FATAL_ERROR "Test failed to run: '${test_command_string}' returned error code: ${ERRCODE}")
  ...
  if (TEXT_BASELINE AND TEXT_RESULT)
    ...
    execute_process(COMMAND ${text_diff_command} RESULT_VARIABLE ERRCODE)
    # Check return value from textual diff command
    if(ERRCODE)
      message("Textual diff failed to run: '${text_diff_command_string}' returned error code: ${ERRCODE}")
    endif()
