...
/*!
  ...
  \author    J. Bakosi
  \date      Tue 29 Jul 2014 09:33:47 AM MDT
...
//! arbitrary types. Note that the model constructor is nullptr and only used to
//! forward its type to the call site inside std::function. The host constructor
//! function is then placed into the factory. This is because Charm++ chares do
//! not explicitly invoke constructors, only call ckNew() on their proxy, which
//! requires all constructor arguments to be present and forwarded to the actual
//! constructor that is only called at a later point in time. This can then be
//! used by those constructors of hosts! that invoke the model constructors'
//! proxies' ckNew() and ignore the std::function. See, e.g.,
//! rngtest::Battery() or the associated unit tests in UnitTest/tests/Base/
//! Factory.h.
template< class Host, class ModelConstructor, class Factory, class Key,
          typename... ModelConstrArgs >
