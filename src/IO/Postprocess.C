...
// -----------------------------------------------------------------------------
// Routine: outparpos - Output particle positions as a new view
// Author : J. Bakosi
// -----------------------------------------------------------------------------
{
  int p;
...
// -----------------------------------------------------------------------------
// Routine: out_3dstep - Output results
// Author : J. Bakosi
// -----------------------------------------------------------------------------
{
...
#ifdef MICROMIXING
static void outpdf(int k, int tav, int onum, int *epdfloc, int *esupel1,
                   int *esupel2, double *c2e,
                   int *psel1, int *psel2, double *parc, double *ce, int *sl,
                   double *tpdf )
// -----------------------------------------------------------------------------
// Routine: outpdf - Output instantaneous temperature PDF
// Author : J. Bakosi
// -----------------------------------------------------------------------------
// Output 'instantaneous' probability density function of temperature
// fluctuations at location k, depending on tav, also outputs time-averaged pdf
//
// k: location index,
...
// -----------------------------------------------------------------------------
{
  ...
  n = 0;
  // loop over elements surrounding nodes of element
  for ( i = esupel2[e]+1; i <= esupel2[e+1]; i++ )
    if ( c2e[esupel1[i]] > EPS )        // only if variance != 0
      // loop over all particles in element esupel1[i]
      for ( j = psel2[esupel1[i]]+1; j <= psel2[esupel1[i]+1]; j++ ) {
        n++;    // incrase number of particles considered
        // calculate standardized temperature
        stp = (parc[psel1[j]]-ce[esupel1[i]])/sqrt(c2e[esupel1[i]]);

        for ( l = 0; l < NBI; l++ )		// count up particles in bins
           if ( (stp > SMIN+l*BINSIZE) && (stp < SMIN+(l+1)*BINSIZE) )
             pdf[l] += 1.0;
      }
  ...
  if ( n ) {    // only if there is anything meaningful to save
    ...
    if (tav) {
      ...
      sprintf( s, "%s.%.4g_%.4g", TPDF_BASE_FILENAME, PL[sl[k]*2+0],
                                  PL[sl[k]*2+1] );
...
static void extract_element_profilepoint_values(
              int e, int i, int *n, int maxn, double maxy, double *yprof,
...
// -----------------------------------------------------------------------------
// Routine: extract_element_profilepoint_values - extract values at a
//          profile-point along a transverse line from element e
// Author : J. Bakosi
// -----------------------------------------------------------------------------
// e : 2d element index,
...
// -----------------------------------------------------------------------------
{
  ...
  // if element bounding stripe
  if ( (DL[i] > coord[A*NDIMN+0]) && (DL[i] < coord[B*NDIMN+0]) ) {
     if ( DL[i] < coord[C*NDIMN+0] ) {	// AC edge
       // if not parallel
       if ( fabs(dtmp=coord[C*NDIMN+0]-coord[A*NDIMN+0]) > EPS )
         y1 = coord[A*NDIMN+1] + (DL[i]-coord[A*NDIMN+0])*
             (coord[C*NDIMN+1]-coord[A*NDIMN+1])/dtmp;
       else
         // if parallel: average yA and yC
         y1 = 0.5*(coord[A*NDIMN+1]+coord[C*NDIMN+1]);
     }
     else // CB edge
       // if not parallel
       if ( fabs(dtmp=coord[C*NDIMN+0]-coord[B*NDIMN+0]) > EPS )
         y1 = coord[B*NDIMN+1] + (DL[i]-coord[B*NDIMN+0])*
             (coord[C*NDIMN+1]-coord[B*NDIMN+1])/dtmp;
       else
         // if parallel: average yC and yB
         y1 = 0.5*(coord[C*NDIMN+1]+coord[B*NDIMN+1]);
         // AB edge
         y2 = coord[A*NDIMN+1] + (DL[i]-coord[A*NDIMN+0])*
             (coord[B*NDIMN+1]-coord[A*NDIMN+1])/
     ...
     NA = (DL[i]*(y[1]-y[2]) +
           x[1]*(y[2]-yprof[in]) +
           x[2]*(yprof[in]-y[1]))/dete[e];
     NB = (x[0]*(yprof[in]-y[2]) +
           DL[i]*(y[2]-y[0]) +
           x[2]*(y[0]-yprof[in]))/dete[e];
     NC = (x[0]*(y[1]-yprof[in]) +
           x[1]*(yprof[in]-y[0]) +
           DL[i]*(y[0]-y[1]))/dete[e];
     ...
     // if edge is aligned with upper one, save the upper intersection point
     // as well
     if ( needboth ) {
        // save the upper one
        yprof[in] = fmax(y1,y2);
...
static void out_profiles(int ndl, int maxn, double maxy, int *es1, int *es2,
                         int *inpoel, double *coord,
		 	 double *uprof, double *vprof, double *u2prof,
                         double *v2prof, double *u3prof,
                         double *v3prof, double *u4prof, double *v4prof,
                         double *uvprof, double *tkeprof,
                         double *epsprof, double *dete, double *yprof,
                         ...
                         #endif
			 const char *cross_filename,
                         const char *downstream_filename )
// -----------------------------------------------------------------------------
// Routine: out_profiles - Output cross-stream profiles at selected locations
// Author : J. Bakosi
// -----------------------------------------------------------------------------
{
  int i, n, e, ie, p;
  ...
  if ( !(dfile = fopen(downstream_filename,"w")) )
  ...
  for ( i = 0; i < ndl; i++ ) { // loop over downstream locations
     ...
     // loop over all elements in stripe
     for ( e = es2[i]+1; e <= es2[i+1]; e++ )
        extract_element_profilepoint_values( es1[e], i, &n, maxn, maxy, yprof,
                                             dete, inpoel, coord, u_, u2_, u3_,
                                             u4_, f_, uprof, vprof, u2prof,
                                             v2prof, u3prof, v3prof, u4prof,
                                             v4prof, uvprof, tkeprof, epsprof
                                             #ifdef MICROMIXING
                                             , &cpeak, &c2peak, c_, c2_, c3_,
                                             c4_, tm_, cprof, c2prof, c3prof,
                                             c4prof, tmprof
                                             #endif
                                           );
     ...
     // y+, <U>, <V>, <uu>, <vv>, skewness u, skewness v, flatness u,
     // flatness v, <uv>, tke, eps, <C>, <c2>, <c3>, <c4>, tm
     for ( e = 0; e < n; e++ )	// loop over profile-points
     {
       ...
       fprintf( cfile, "%g\t%g\t%g\t%g\t%g\t%g\t%g\t%g\t%g\t%g\t%g\t%g"
                       ...
                       #endif
                       "\n",
                       yprof[ie], uprof[ie], vprof[ie], u2prof[ie], v2prof[ie],
                       u3prof[ie], v3prof[ie], u4prof[ie], v4prof[ie],
                       uvprof[ie], tkeprof[ie], epsprof[ie]
                       #ifdef MICROMIXING
                       , cprof[ie]/cpeak, c2prof[ie]/c2peak, c3prof[ie],
                       c4prof[ie], tmprof[ie]
                       #endif
              );
     }
...
// -----------------------------------------------------------------------------
// Routine: extract_element_profilepoint_values_c -
...
// -----------------------------------------------------------------------------
{
  ...
  // compute edge-intersections
  // if element in bounding stripe
  if ( (0.0 > coord[AN+1]) && (0.0 < coord[BN+1]) )
  {
     ...
     // AB edge
     x2 = coord[AN+0] - coord[AN+1]*(coord[BN+0]-coord[AN+0]) /
                                    (coord[BN+1]-coord[AN+1]);

     // if an edge is aligned with the outflow, count both intersection points
     if ( fabs(coord[AN+0]-maxx)<EPS )
     {
       if ( (fabs(coord[BN+0]-maxx)<EPS) || (fabs(coord[CN+0]-maxx)<EPS) )
         needboth = 1;
     }
     else
       if ( (fabs(coord[BN+0]-maxx)<EPS) && (fabs(coord[CN+0]-maxx)<EPS) )
...
static void out_centerline_profiles(int ec_size, int *ec, double maxx,
                                    int *inpoel, double *coord,
                                    double *xprof_c, double *uprof_c,
                                    double *pprof_c, double *dete,
                                    double *u_, double *pr_,
                                    const char *centerline_filename )
// -----------------------------------------------------------------------------
// Routine: out_centerline_profiles - Output centerline streamwise profiles
//                                    behind the cylinder
// Author : J. Bakosi
// -----------------------------------------------------------------------------
{
  ...
  for ( n=e=0; e < ec_size; e++ )// loop over all elements in centerline stripe
    extract_element_profilepoint_values_c( ec[e], &n, maxx, xprof_c, uprof_c,
                                           pprof_c, dete, inpoel, coord,
                                           u_, pr_ );
...
static void out_wall_profiles(int angle_size, int *inprof_w, double *angle_w,
                              double *pr_, double *du_,
                              const char *wall_filename)
// -----------------------------------------------------------------------------
// Routine: out_wall_profiles - Output profiles along the cylinder wall
// Author : J. Bakosi
// -----------------------------------------------------------------------------
{
  int p, p4;
  ...
  {
    p4 = inprof_w[p]*4;
    fprintf( wfile, "%g\t%g\t%g\n", angle_w[p], 2.0*pr_[inprof_w[p]],
                                    -0.5*(du_[p4+2]-du_[p4+1])/sqrtREp2 );
  }
...
// -----------------------------------------------------------------------------
// Routine: out_az_profiles - Output profiles along azimuthal lines
// Author : J. Bakosi
// -----------------------------------------------------------------------------
{
  ...
  {
    // construct filename for aizumthal direction
    sprintf( s, "%s.%.4g", az_base_filename, AZ[z] );
    if ( !(azfile[z] = fopen(s,"w")) ) ERR("Cannot open file\n");
  }

  // output azimuthal profiles
  #ifdef _OPENMP
  #pragma omp parallel for private(z,zN,m,mN,j,e,eN,A,B,C,AN,BN,CN,AU,BU,\
                                   CU,v,v2,v2w,k,k2,t)
  #endif
  for ( z = 0; z < MAXAZ; z++ )	// loop over all azimuthal angles
  {
    ...
    // loop over all element intersected by angle z
    for ( m=0, j=az2[z]+1; j <= az2[z+1]; j++ ) {
...
// -----------------------------------------------------------------------------
// Routine: intersect - Compute intersection
// Author : J. Bakosi
// -----------------------------------------------------------------------------
// compute the intersection of a line (starting from (0,0) going through "f")
// and an edge given by points "a" and "b"
...
static void out_spatially_averaged_fields(
             int npoin, int nbpoin, double t, double dt, int minnp, int pit,
             double *drag, double *lift, double *u, int *betags, int *binpoel,
             double *wenr, double *wel, double *pr, double *du, double *u2,
             double *f
	     #ifndef WALLFUNCTIONS
	     , int rit
	     #endif
...
// -----------------------------------------------------------------------------
// Routine: out_spatially_averaged_fields - Output spatially-averaged fields at
//                                          time t
// Author : J. Bakosi
// -----------------------------------------------------------------------------
{
  ...
  #ifdef _OPENMP
    #ifdef MICROMIXING
      #pragma omp parallel for private(p,pU,pN) \
                           reduction(+:au,av,uu,vv,ww,uv,uw,vw,eps,ap,ac,ac2)
    #else
      #pragma omp parallel for private(p,pU,pN) \
                           reduction(+:au,av,uu,vv,ww,uv,uw,vw,eps,ap)
    #endif // MICROMIXING
  #endif // _OPENMP
  for ( p = 0; p < npoin; p++ )
  {
  ...
  if ( !(opos = fopen(OUT_POSTPROCESS_TIME_FILENAME,"a")) )
    ERR("Cannot open OUT_POSTPROCESS_TIME_FILENAME");
  //  1: time, <U>, <V>, <uu>, <vv>, <ww>, <uv>, <uw>, <vw>, eps, pr, minnpel,
  //     dt, <C>, <cc>,
  //     #ifndef WALLFUNCTIONS
  //     16: rit,
  //     #endif
...
static void comp_time_averaged_fields(
              int npoin, double *u, double *pr, double *du, double *tu,
              double *tdu, double *tpr, double *drag, double *lift,
              double *tdrag, double *tlift, double *u2, double *u3, double *u4,
              double *f, double *tu2, double *tu3, double *tu4, double *tf,
              #ifdef MICROMIXING
	      double *c, double *c2, double *c3, double *c4, double *tm,
	      ...
	      int *onum )
// -----------------------------------------------------------------------------
// Routine: compute_time_averaged_fields - Computes time-averaged fields
// Author : J. Bakosi
// -----------------------------------------------------------------------------
{
  int p, pN, pU, p4;
  ...
  for ( p = 0; p < npoin; p++ )
  {
    ...
    else
    {
      ...
      tuc[p*3+0] = ((double)((*onum)-1)/(*onum))*(tuc[p*3+0] + uc[p*3+0]/
                   ((*onum)-1));
      tuc[p*3+1] = ((double)((*onum)-1)/(*onum))*(tuc[p*3+1] + uc[p*3+1]/
                   ((*onum)-1));
      tuc[p*3+2] = ((double)((*onum)-1)/(*onum))*(tuc[p*3+2] + uc[p*3+2]/
                   ((*onum)-1));
      #endif
    }
  }
...
void out_inst_dist(int npoin, int nbpoin, int minnp, int pit, int maxn,
                   double t, double dt, double *drag, double *lift,
                   double *tdrag, double *tlift, int *betags, int *binpoel,
                   int *onum, double *u, double *wenr, double *wel, double *du,
                   int ndl, double maxx, double maxy, int *es1, int *es2,
                   int ec_size, int *ec, int *az1, int *az2, double *xprof_c,
                   double *uprof_c, double *pprof_c, int *inpoel, double *coord,
                   double *uprof, double *vprof, double *u2prof, double *v2prof,
                   double *u3prof, double *v3prof, double *u4prof,
                   double *v4prof, double *uvprof, double *tkeprof,
                   double *epsprof, double *dete, double *yprof, int angle_size,
                   int *inprof_w, double *angle_w, double *r_az, double *dir_az,
                   double *N_az,
                   #ifndef WALLFUNCTIONS
		   int rit,
		   #endif
		   #ifdef MICROMIXING
		   double *c, double *c2, double *c3, double *c4, double *tm,
                   double *uc, double *tc, double *tc2, double *tc3,
                   double *tc4, double *ttm, double *tuc, int npl, int *epdfloc,
                   int *esupel1, int *esupel2, double *c2e, int *psel1,
                   int *psel2, double *parc, double *ce, int *sl, double *tpdf,
                   double *cprof, double *c2prof, double *c3prof,
                   double *c4prof, double *tmprof,
                   #endif
		   double *pr, double *tu, double *u2, double *u3, double *u4,
                   double *f, double *tu2, double *tu3, double *tu4, double *tf,
                   double *tpr, double *tdu )
// -----------------------------------------------------------------------------
// Routine: out_inst_dist - Output 'instantaneous' statistics,
//                          compute and outputs time-averaged statistics
// Author : J. Bakosi
// -----------------------------------------------------------------------------
{
  #ifdef MICROMIXING
  ...
  out_spatially_averaged_fields(npoin, nbpoin, t, dt, minnp, pit, drag, lift,
                                u, betags, binpoel, wenr, wel, pr, du, u2, f
                                #ifndef WALLFUNCTIONS
                                , rit
                                #endif
                                #ifdef MICROMIXING
  ...
  out_profiles(ndl, maxn, maxy, es1, es2, inpoel, coord, uprof, vprof, u2prof,
               v2prof, u3prof, v3prof, u4prof, v4prof, uvprof, tkeprof, epsprof,
               dete, yprof, u, u2, u3, u4, f,
               #ifdef MICROMIXING
  ...
  out_centerline_profiles(ec_size, ec, maxx, inpoel, coord, xprof_c, uprof_c,
                          pprof_c, dete, u, pr,
  ...
  if (t > AVTIME) {
    ...
    comp_time_averaged_fields(npoin, u, pr, du, tu, tdu, tpr, drag, lift, tdrag,
                              tlift, u2, u3, u4, f,
                              tu2, tu3, tu4, tf,
                              #ifdef MICROMIXING
    ...
    #endif
    for ( p = 0; p < npl; p++ )
      outpdf(p, 1, *onum, epdfloc, esupel1, esupel2, c2e, psel1, psel2, parc,
             ce, sl, tpdf );
    #endif
  
    // output time-averaged profiles at downstream locations
    out_profiles(ndl, maxn, maxy, es1, es2, inpoel, coord, uprof, vprof, u2prof,
                 v2prof, u3prof, v3prof, u4prof, v4prof, uvprof, tkeprof,
                 epsprof, dete, yprof, tu, tu2, tu3, tu4, tf,
                 #ifdef MICROMIXING
    ...
    out_az_profiles(az1, az2, inpoel, tu, tu2, r_az, dir_az, N_az,
                    TAZ_BASE_FILENAME );
  }
  else
  {
    ...
    #endif
    for ( p = 0; p < npl; p++ )
      outpdf(p, 0, *onum, epdfloc, esupel1, esupel2, c2e, psel1, psel2, parc,
             ce, sl, tpdf );
    #endif
  }
}
