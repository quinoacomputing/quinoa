...
// }
  ...
  private:
    ...
    std::vector< std::pair< int, std::set< tk::UnsMesh::Edge > > > m_reqed;
    ...
    //! \brief PEs associated to newly added edges during initial uniform mesh
    //!   refinement
    std::unordered_map< tk::UnsMesh::Edge,
                        int,
                        tk::UnsMesh::EdgeHash,
                        tk::UnsMesh::EdgeEq > m_edgePEs;
...
// std::cout << '\n' << CkMyPe() << " newid, (o:n): ";
// for (const auto& c : m_newid) std::cout << c.first << ':' << c.second << ' ';
// std::cout << '\n';
      ...
      for (auto& c : m_msum)
        for (auto& s : c.second) {
          decltype(s.second) n;
          for (auto p : s.second) {
            auto it = m_newid.find(p);
            if (it != end(m_newid)) n.insert( it->second );
          }
          s.second = std::move( n );
        }
