...
// *****************************************************************************
{
  ...
  for (const auto& s : g_newinputdeck.template get< newtag::ale, newtag::dirichlet >()) {
    auto k = bface.find(s);
    if (k != end(bface)) {
  ...
  for (const auto& s : g_newinputdeck.template get< newtag::ale, newtag::symmetry >()) {
    auto k = bnode.find(s);
    if (k != end(bnode)) {
...
// *****************************************************************************
{
  ...
  auto meshveltype = g_newinputdeck.get< newtag::ale, newtag::mesh_velocity >();
  if (meshveltype == ctr::MeshVelocityType::SINE) {
    ...
    for (auto j : g_newinputdeck.get< newtag::ale, newtag::mesh_motion >())
...
// *****************************************************************************
{
  ...
  if (smoother == ctr::MeshVelocitySmootherType::LAPLACE) {
    ...
    if (maxv > 1.0e-8) {
      auto mult = g_newinputdeck.get< newtag::ale, newtag::vortmult >();
      for (auto j : g_newinputdeck.get< newtag::ale, newtag::mesh_motion >())
        for (std::size_t p=0; p<m_vorticity[0].size(); ++p)
...
// *****************************************************************************
{
  ...
  if (smoother == ctr::MeshVelocitySmootherType::LAPLACE) {
    ...
    auto a1 = g_newinputdeck.get< newtag::ale, newtag::vortmult >();
    for (auto j : g_newinputdeck.get< newtag::ale, newtag::mesh_motion >())
...
// *****************************************************************************
{
  ...
  const auto& f = g_newinputdeck.get< newtag::ale, newtag::meshforce >();
  m_wf.fill( 0.0 );
  for (std::size_t e=0; e<m_inpoel.size()/4; ++e) {
    // access node IDs
    const std::array< std::size_t, 4 >
...
// *****************************************************************************
{
  ...
  const auto& sbc = g_newinputdeck.get< newtag::ale, newtag::symmetry >();
