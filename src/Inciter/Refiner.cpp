...
// *****************************************************************************
{
  ...
  // other chares.
  for (const auto& [ edge, edgeref ] : ref_edges) {
    const auto& ed = edge.get_data();
    const auto ged = Edge{{ m_gid[ ed[0] ], m_gid[ ed[1] ] }};
    m_localEdgeData[ ged ] = { edgeref.needs_refining, edgeref.lock_case };
  }
  ...
  for (auto i : m_refiner.tet_store.intermediate_list) {
...
// *****************************************************************************
{
  ...
  const auto& tet_store = m_refiner.tet_store;
  for (const auto& [ face, tetid ] : bndFaceTets) {
