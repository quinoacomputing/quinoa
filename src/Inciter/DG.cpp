...
DG::DG( const CProxy_Discretization& disc,
  ...
  m_lhs( m_u.nunk(),
         g_newinputdeck.get< newtag::ndof >()*
         g_newinputdeck.get< newtag::ncomp >() ),
  m_rhs( m_u.nunk(), m_lhs.nprop() ),
  m_mtInv(
    tk::invMassMatTaylorRefEl(g_newinputdeck.get< newtag::rdof >()) ),
  m_uNodalExtrm(),
  m_pNodalExtrm(),
  m_uNodalExtrmc(),
  m_pNodalExtrmc(),
...
// *****************************************************************************
{
  ...
  const auto pref = g_newinputdeck.get< newtag::pref, newtag::pref >();
  if( pref )
  {
    const auto ndofmax = g_newinputdeck.get< newtag::pref, newtag::ndofmax >();
...
// *****************************************************************************
{
  ...
  if (pref && m_stage == 0 && d->T() > 0)
    g_dgpde[d->MeshId()].eval_ndof( myGhosts()->m_nunk, myGhosts()->m_coord,
                  ...
                  g_newinputdeck.get< newtag::pref, newtag::indicator >(),
                  g_newinputdeck.get< newtag::ndof >(),
...
// *****************************************************************************
{
  ...
  if (rdof > 1) {
    ...
    if (g_newinputdeck.get< newtag::limsol_projection >())
...
// *****************************************************************************
{
  ...
  if (m_stage == 0)
  {
    ...
    if (std::abs(const_dt) > eps) {
      ...
      mindt *= g_newinputdeck.get< newtag::cfl >();
    }
  }
  else
  {
...
// *****************************************************************************
{
  ...
  m_uNodalExtrm.resize( Disc()->Bid().size(), std::vector<tk::real>( 2*
    m_ndof_NodalExtrm*g_newinputdeck.get< newtag::ncomp >() ) );
  m_pNodalExtrm.resize( Disc()->Bid().size(), std::vector<tk::real>( 2*
    m_ndof_NodalExtrm*m_p.nprop()/g_newinputdeck.get< newtag::rdof >()));
...
// *****************************************************************************
{
  return g_newinputdeck.get< newtag::cmd, tag::io, tag::refined >() &&
         g_newinputdeck.get< newtag::scheme >() != ctr::SchemeType::DG;
}
...
// *****************************************************************************
{
  ...
  if (g_newinputdeck.get< newtag::pref, newtag::pref >()) {
