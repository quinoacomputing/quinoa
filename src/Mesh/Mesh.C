...
//  ------------------------------------------------------------------------------------------------------------
//
//  Unstructured 2D mesh preprocessing functions,
//  for more info see main.cc
//
//  J. Bakosi, February 2007
//
...
// OUT_MESH_FILENAME, where the reordered (and finally used) mesh will be written
//
// arguments:
//   o npoin: the total the number of points (nodes) read in from the meshfile,
//   o nbpoin: the total number of boundary points/elements read in from the meshfile,
//   o nelem: the total number of domain (triangular) elements read in from the meshfile,
//   o coord[npoin*NDIMN]: matrix that stores x,y coordinates of all points,
//   o binpoel[nelem_g*NBNODE]: matrix that stores the global element connectivity
//     indices of boundary elements,
//   o inpoel[nelem_g*NNODE]: matrix that stores the global element connectivity indices
//     of domain elements,
//   o bptags[nbpoin*3]: matrix that stores the boundary tags assigned to boundary points,
//   o betags[nelem_g*2]: matrix that stores the boundary tags assigned to boundary elements.
//  
//  the output arrays are allocated dynamically in this function, so the caller needs
//  to take care of freeing them
//
{
  FILE *imesh, *omesh, *opos3d, *igeo;
  char str[STRLEN];
  int meshversion, meshtype, meshdoublesize, e, p, etype, ntags, itmp, num, i, q;
  double dtmp;


  // start writing omesh...
  if (!(omesh = fopen(OUT_MESH_FILENAME,"w"))) ERR("cannot open OUT_MESH_FILENAME!");

  // start reading mesh data in...
  if (!(imesh = fopen(INP_MESH_FILENAME,"r"))) ERR("cannot open INP_MESH_FILENAME!");

  fgets( str, STRLEN, imesh );				// str << "$MeshFormat\n"
  fprintf( omesh, "%s", str );				// copy to omesh

  fscanf( imesh, "%d %d %d\n", &meshversion, &meshtype, &meshdoublesize );	// 2 0 8
  fprintf( omesh, "%d %d %d\n", meshversion, meshtype, meshdoublesize );	// copy to omesh
  if ( (meshversion != 2) || (meshtype != 0) || (meshdoublesize != sizeof(double)) )
  ...
  if ( !(pg = (int*)malloc((*npoin)*sizeof(int))) ) ERR("Can't allocate memory!");

  // array for storing x and y coordinates of vertices,
  if ( !(*coord = (double*)malloc((*npoin)*NDIMN*sizeof(double))) ) ERR("Can't allocate memory!");

  for ( p = 0; p < *npoin; p++ )
  {
    // read in coordinates of point p and throw away z coordinate...
    fscanf( imesh, "%d %lf %lf %lf\n", pg+p, *coord+p*NDIMN+0, *coord+p*NDIMN+1, &dtmp );
  }

  // check if numbering starts from 0...
  for ( itmp = p = 0; p < *npoin; p++ ) if (pg[p] == 0) itmp = 1;
  if (!itmp) ERR("point numbering should start with 0, create a Point(0) in the gmsh .geo file");

  fgets( str, STRLEN, imesh );	// str << "$EndNodes\n"
  fgets( str, STRLEN, imesh );	// str << "$Elements\n"
  // read in Nelem_g = number of boundary (line) elements + number of domain elements (triangles)
  ...
  // don't know this yet...
  if ( !(etags = (int*)malloc(nelem_g*2*sizeof(int))) ) ERR("Can't allocate memory!");
  if ( !(*betags = (int*)malloc(nelem_g*2*sizeof(int))) ) ERR("Can't allocate memory!");
  if ( !(*binpoel = (int*)malloc(nelem_g*NBNODE*sizeof(int))) ) ERR("Can't allocate memory!");
  if ( !(*inpoel = (int*)malloc(nelem_g*NNODE*sizeof(int))) ) ERR("Can't allocate memory!");
    ...
    switch ( etype )
    {
      case 1: // line (boundary element with 2 nodes)...
	      ...
	      // store boundary element connectivity indices in binpoel...
              fscanf( imesh, "%d %d\n", *binpoel+(*nbpoin)*NBNODE+0, *binpoel+(*nbpoin)*NBNODE+1 );
      ...
      case 2: // triangle (surface element with 3 nodes)
	      ...
	      // store three node indices in inpoel...
    	      fscanf( imesh, "%d %d %d\n", *inpoel+(*nelem)*NNODE+0, *inpoel+(*nelem)*NNODE+1,
  ...
  // array for containing the above: the first column is the global indices of the
  // boundary points, the other two are the physical and elementary geometrical tags
  if ( !(*bptags = (int*)malloc((*nbpoin)*3*sizeof(int))) ) ERR("Can't allocate memory!");
  ...
  // start writing postprocess-file (specified by OUT_POSTPROCESS_3D_FILENAME) by
  // copying file specified by INP_GEO_FILENAME in it...
  if (!(opos3d = fopen(OUT_POSTPROCESS_3D_FILENAME,"w"))) ERR("cannot open OUT_POSTPROCESS_3D_FILENAME!");

  if (!(igeo = fopen(INP_GEO_FILENAME,"r"))) ERR("cannot open INP_GEO_FILENAME!");
...
static void writemeshfile( int npoin, int nelem, int nbpoin, double *coord, int *inpoel )

//
// writes out the finally used mesh
//
{
  int p, e, cnt;
  FILE *omesh;
  ...
  if (!(omesh = fopen(OUT_MESH_FILENAME,"a"))) ERR("cannot open OUT_MESH_FILENAME!");
  ...
  for ( e = 0; e < nelem; e++ )		// triangle elements...
  {
    ...
    // three point numbers...
    fprintf( omesh, "%d %d %d\n", inpoel[e*NNODE+0], inpoel[e*NNODE+1], inpoel[e*NNODE+2] );
...
//
{
  ...
  if ( !(*esup2 = (int*)malloc((npoin+1)*sizeof(int))) ) ERR("Can't allocate memory!");
  
  // calculate elements surrounding points: esup1, esup2...
  // element pass 1: count number of elements connected to each point...
  for ( i = 0; i < npoin+1; i++ ) (*esup2)[i] = 0;	// initialize esup2
  for ( e = 0; e < nelem; e++ )
  {
  ...
  if ( !(*esup1 = (int*)malloc(size*sizeof(int))) ) ERR("Can't allocate memory!");
  ...
  if ( !(*psup2 = (int*)malloc((npoin+1)*sizeof(int))) ) ERR("Can't allocate memory!");
  
  // temporary array, used only in this function...
  if ( !(lpoin = (int*)malloc(npoin*sizeof(int))) ) ERR("Can't allocate memory!");

  // determine size...
  for ( i = 0; i < npoin; i++ ) lpoin[i] = 0;
  (*psup2)[0] = 0;
  size = 0;
  for ( p = 0; p < npoin; p++ )
  {
  ...
  if ( !(*psup1 = (int*)malloc((size+1)*sizeof(int))) ) ERR("Can't allocate memory!");
...
static void regensup( int npoin, int nelem, int *inpoel,
	              int *esup1, int *esup2, int *psup1, int *psup2 )
//
// regenerates derived data structures for:
//   o elements surrounding points (linked lists: esup1, esup2),
//   o points surroinding points (linked lists: psup1, psup2),
//
// input arguments:
...
//
{
  ...
  if ( !(*bpg = (int*)malloc(nbpoin*sizeof(int))) ) ERR("Can't allocate memory!");
  
  for ( size=1, i=0; i < nbpoin; i++ )		// loop over boundary elements (equal amount of boundary points)
  {
    (*bpg)[i] = binpoel[i*NBNODE+0];		// generate boundary->global index mapping array
  ...
  if ( !(*bpsup1 = (int*)malloc(size*sizeof(int))) ) ERR("Can't allocate memory!");
  if ( !(*bpsup2 = (int*)malloc((nbpoin+1)*sizeof(int))) ) ERR("Can't allocate memory!");
  ...
  {					// loop over points surrounding boundary point i
    for ( j = psup2[(*bpg)[i]]+1; j <= psup2[(*bpg)[i]+1]; j++ )
      (*bpsup1)[q++] = psup1[j];	// copy out indices of points surrounding boundary point i
    (*bpsup2)[i+1] = q-1;		// put in ending index of boundary point surrounding point indices
  }
}
...
//
{
  ...
  // find out maximum size of "redundant" elements surrounding points of elements (resupel): mresupel
  for ( mresupel = e = 0; e < nelem; e++ )
  {
    ...
    // count up "redundant" number of elements surrounding all three nodes of element e
    j = esup2[A+1]-esup2[A] + esup2[B+1]-esup2[B] + esup2[C+1]-esup2[C];
    if ( j > mresupel ) mresupel = j;	// find out the biggest one
  }

  // help array for storing indices of elements surrounding nodes of an element in a
  // "redundant" way (resupel) and in a "non-redundant" way (nresupel),
  // (both of size mresupel for worst case of nresupel)
  if ( !(resupel = (int*)malloc(mresupel*sizeof(int))) ) ERR("Can't allocate memory!");
  if ( !(nresupel = (int*)malloc(mresupel*sizeof(int))) ) ERR("Can't allocate memory!");
  ...
  for ( mesupel = e = 0; e < nelem; e++ )
  {
    ...
    for ( l = i = 1; i < j; i++ )
    {
       ...
       for ( haveit = k = 0; k < l; k++ ) if (nresupel[k] == resupel[i]) { haveit = 1; k = l; }

       if ( !haveit )	// put resupel[i] into nresupel if new
	 nresupel[l++] = resupel[i];
    }

    mesupel += l-1;	// add up number of "non-redundant" elements surrounding nodes of elements
    			// -1, because we don't store element e itself (but it's in resupel)
  }
  
  // linked lists for storing elements surrounding points of elements: esupel1, esupel2
  if ( !(*esupel1 = (int*)malloc(mesupel*sizeof(int))) ) ERR("Can't allocate memory!");
  if ( !(*esupel2 = (int*)malloc((nelem+1)*sizeof(int))) ) ERR("Can't allocate memory!");
  ...
  for ( e = 0; e < nelem; e++ )
  {
    ...
    // store nresupel of element e in global linked lists (esupel1, esupel2), except element e itself
    (*esupel2)[e+1] = (*esupel2)[e] + l - 1;		// -1: don't store element e
    for ( k = i = 0; i < l; i++ ) if (nresupel[i] != e) (*esupel1)[(*esupel2)[e]+(k++)+1] = nresupel[i];
  }
...
//
// output some information about filenames and number of elemente/points to stdout
//
{
  printf( "--------------------------------------------------------------------------\n" );
...
static void correctordering( int npoin, int nelem, int nbpoin, int *inpoel, int *binpoel )
//
// corrects the ordering, gmsh doesn't necessarily sort the numbering of the nodes and/or elements,
// this is not usual, however, I have encountered it, so I correct it here (if they're in the right order,
// nothing happens), new versions of gmsh have a feature to order at save, so this should not be necessary
// anymore, but it's fast, done only once in the beginning and doesn't hurt
//
// input arguments:
...
//
{
  ...
  for ( maxpg = p = 0; p < npoin; p++ ) if (maxpg < pg[p]) maxpg = pg[p];
    
  // temporary array for inverse mapping of pg
  if ( !(ipg = (int*)malloc((maxpg+1)*sizeof(int))) ) ERR("Can't allocate memory!");
...
//   o inpoel[nelem*NNODE]: the updated domain-element connectivity,
//   o binpoel[nbpoin*NBNODE]: the updated boundary-element connectivity,
//   o coord[npoin*NDIMN]: the update matrix that stores x,y coordinates of all points,
//   o bptags[nbpoin*3]: matrix that stores the boundary tags assigned to boundary points.
//
{
  int i, j, e, p, q; 
  ...
  // allocate memory
  if ( !(lpoin = (int*)calloc(npoin, sizeof(int))) ) ERR( "Cannot allocate memory for lpoin");
  if ( !(hpoin = (int*)calloc(npoin, sizeof(int))) ) ERR( "Cannot allocate memory for hpoin");
  if ( !(kpoin = (int*)calloc(npoin, sizeof(int))) ) ERR( "Cannot allocate memory for kpoin");
  if ( !(mapvec = (int*)calloc(npoin, sizeof(int))) ) ERR( "Cannot allocate memory for mapvec");
  ...
  while ( num < npoin )
  {
    ...
    while ( (p = hpoin[i]) != -1 )
    {
      ...
      {
        q = psup1[j];
        if ( lpoin[q] != 1 ) 	// loop only through those points that have not been counted yet
        {
          mapvec[q] = num++;
  ...
  // apply mapping to boundary element conectivity and boundary point tag indices after renumbering
  for ( e = 0; e < nbpoin; e++ )
  {
...
static void precompute_element_data( int nelem, int npoin, int *inpoel, double *coord, int *esup1,
                                     int *esup2, double **Ae, double **dNx, double **dNy,
			             double **dete, double **sqrtAp, double *minsqrtAp )
//
// precalculates:
//   * element sizes,
...
//   o sqrtAp[npoin]: average of the square roots of the elements surrounding each point.
...
//
{
  ...
  if ( !(*Ae = (double*)malloc(nelem*sizeof(double))) ) ERR("Can't allocate memory!");
  if ( !(*dNx = (double*)malloc(nelem*NNODE*sizeof(double))) ) ERR("Can't allocate memory!");
  if ( !(*dNy = (double*)malloc(nelem*NNODE*sizeof(double))) ) ERR("Can't allocate memory!");
  if ( !(*dete = (double*)malloc(nelem*sizeof(double))) ) ERR("Can't allocate memory!");
  ...
  // for storing the characteristic element size (square-root of average element areas
  // surrounding points)
  if ( !(*sqrtAp = (double*)malloc(npoin*sizeof(double))) ) ERR("Can't allocate memory!");
  ...
  for ( p = 0; p < npoin; p++ )		// loop over all owned points
  {
    ...
    (*sqrtAp)[p] = sqrt((*sqrtAp)[p]/n);// calculate square-root of average element size
  }
...
static void genesuel( int nelem, int *inpoel, int *esupel1, int *esupel2, int **esuel )
//
// generates derived data structure to store elements surrounding elements
//
{
  int e, f, A, B, C;
  ...
  if ( !(*esuel = (int*)malloc(nelem*NNODE*sizeof(int))) ) ERR("Can't allocate memory!");

  // initialize esuel with -1 indicating no element on that side
  for ( e = 0; e < nelem*NNODE; e++ ) (*esuel)[e] = -1;
  ...
  for ( e = 0; e < nelem; e++ )				// loop over all elements
  {
    ...
    B = inpoel[e*NNODE+1];
    C = inpoel[e*NNODE+2];
    for ( f = esupel2[e]+1; f <= esupel2[e+1]; f++ )	// loop over elements surround nodes of element e
    {
      if ( ((A==inpoel[esupel1[f]*NNODE+0]) ||
	    ...
	    (B==inpoel[esupel1[f]*NNODE+2])) ) (*esuel)[e*NNODE+2] = esupel1[f];
      ...
      if ( ((B==inpoel[esupel1[f]*NNODE+0]) ||
	    ...
	    (C==inpoel[esupel1[f]*NNODE+2])) ) (*esuel)[e*NNODE+0] = esupel1[f];
      ...
      if ( ((C==inpoel[esupel1[f]*NNODE+0]) ||
	    ...
	    (C==inpoel[esupel1[f]*NNODE+2])) &&
           ((A==inpoel[esupel1[f]*NNODE+0]) ||
            (A==inpoel[esupel1[f]*NNODE+1]) ||
	    (A==inpoel[esupel1[f]*NNODE+2])) ) (*esuel)[e*NNODE+1] = esupel1[f];
    }
  }
}
...
//
// frees temporary arrays needed for mesh preprocessing
//
{
  free( pg );
  free( etags );
...
//
{
  ...
  // regenerate derived data structures for elements and points surrounding points after renumbering
  ...
  // generate derived data structure to store elements surrounding points of elements
