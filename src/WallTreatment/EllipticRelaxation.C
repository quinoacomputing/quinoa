...
static void build_lhs(int nelem, int nbpoin, int nthreads, int *bpg, int *inpoel,
                      int *bpsup1, int *bpsup2, int *bptags, double *Ae,
                      double *dNx, double *dNy, double *u2, double *f,
                      double *wnz, sparsemat *R)
// -----------------------------------------------------------------------------
// Routine: build_lhs - Build lhs for elliptic relaxation step
// Author : J. Bakosi
// -----------------------------------------------------------------------------
{
  ...
  #ifdef _OPENMP
  #pragma omp parallel for private(e,eN,A,B,C,UA,UB,UC,tke,eps,l1,l2,i,L,aL2,  \
                                   dtmp)
  #endif
  ...
  for (cw=i=0; i < nbpoin; i++) // loop over all boundary points
    if (bptags[i*3+1] == 4)     // wall
      // loop over nonzeros in block row bpg[i]
      for (j=bpsup2[i]+1; j<=bpsup2[i+1]; j++)
        for (k=0; k<RDOF; k++)  // loop over all degrees of freedom
          // save matrix value, increase wall counter
          wnz[cw++] = getmr(R,bpg[i],bpsup1[j],k);
  ...
  #pragma omp parallel for private(i,k,j)
  #endif
  // modify matrix for Dirichlet conditions for top homogeneous Dirichlet for
  // off-diagonal (diagonal remains homogeneous Neumann)
  for (i=0; i<nbpoin; i++)      // loop over all boundary points
    if (bptags[i*3+1] != 4) {   // not wall
       ...
       for (j=bpsup2[i]+1; j<=bpsup2[i+1]; j++)
         for ( k = 1; k < RDOF-1; k++ )
           if (k!=4)                            // touch only off-diagonal
             insmr(R,bpsup1[j],bpg[i],k,0.0);   // insert zero
...
// -----------------------------------------------------------------------------
// Routine: rstep - Elliptic relaxation step
// Author : J. Bakosi
// -----------------------------------------------------------------------------
...
// -----------------------------------------------------------------------------
{
  ...
  double tke[NNODE], au2[U2DOF], ba[U2DOF], adu[NDIMN*NDIMN], nr[NDIMN],
         dc[RDOF];
  ...
  #ifdef _OPENMP
  #pragma omp parallel for private(e,eN,j,A,B,C,UA,UB,UC,RA,RB,RC,tke,ak,ak2,  \
                                   af,au2,ba,det,Av,adu,dtmp)
  #endif
  for (e=0; e<nelem; e++) {
    ...
    // wiggly-p_12
    dtmp = Ae[e]/3.0*ak*((C2*Av+GAMMA5/3.0)*adu[1] -
                          GAMMA5/3.0*adu[2] +
                          GAMMA5*ba[0]*(adu[1]-adu[2]));
    #ifdef _OPENMP
    #pragma omp atomic
    #endif
    rrhs[RA+1] += dtmp;
    ...
    // wiggly-p_21
    dtmp = Ae[e]/3.0*ak*((C2*Av+GAMMA5/3.0)*adu[2] -
                          GAMMA5/3.0*adu[1] +
                          GAMMA5*ba[1]*(adu[2]-adu[1]));
    #ifdef _OPENMP
    #pragma omp atomic
    #endif
    rrhs[RA+3] += dtmp;
  ...
  //   o substract column (i) of the matrix multiplied by the value of the
  //     boundary condition (xi) from the rhs
  //     (ie. for loop over j: rhsj -= aji*xi),
  //   o put 1.0 in the main diagonal,
  //   o zero everything else in the row and column corresponding to the
  //     boundary point index (i),
  //   o put the boundary value (xi) into rhs (ie. ri = xi)
  // wall: inhomogeneous Dirichlet for wiggly-p_ij (i,j=1,2), homogeneous for
  ...
  for (cw=i=0; i<nbpoin; i++)   // loop over boundary points
    if (bptags[i*3+1] == 4) {   // wall
      ...
      for (j=bpsup2[i]+1; j<=bpsup2[i+1]; j++)
        for (k=0; k<RDOF; k++)  // loop over all degrees of freedom
          // substract from rhs, increase wall counter
          rrhs[bpsup1[j]*RDOF+k] -= dc[k]*wnz[cw++];
  ...
  // top: homogeneous Dirichlet for off-diagonal
  // (diagonal remains homogeneous Neumann)
  #ifdef _OPENMP
