...
//
//  Contains the main timestepping loop, the main loop that updates particles and implements particle
//  boundary conditions, in essence, this file contains the core of the whole program.
...
//
// compute distance of particle p from the source
//
{
  register int p2 = p*2;

  return( sqrt((parcoord[p2+0]-XS)*(parcoord[p2+0]-XS) + (parcoord[p*2+1]-YS)*(parcoord[p*2+1]-YS)) );
}
#endif
*/
...
#ifdef WALLFUNCTIONS
static void parwallfunctionbc( int p, double tke, double af, double *au, double *au2, double *dp,
                               double *rBA, double *nro, double *parvel, double *parfreq )
//
// enforces wall boundary condition on velocity and frequency of particle p
...
static void lstep( int stage, int nelem, int npar, int nwe, int nthreads, int *inpoel, int *impcnt,
                   int *elp, int *npeldt, int *wec, int *esuel, int *npel, int *esupel1, int *esupel2,
		   double dt, double minx, double miny, double maxx, double maxy,
		   double *parcoord, double *parvel,
		   double *dNx, double *dNy, double *wrBA, double *wrA, double *wel,
		   double *wenr, double *du,
		   double *Ae, double *pr, double *rg,
		   double *coord, int *wallstruck, int *psel1, int *psel2, double *u, double *parfreq,
                   double *f, double *u2, int *bruteforced
		   #ifndef WALLFUNCTIONS
		   , double *ddu, double *ru, double *avc0, int *wA, double *rho, VSLStreamStatePtr *stream
		   #endif
		   #ifdef MICROMIXING
...
//
{
  int eN, j, jN, p, p2, p3, pNGRPP, A, B, C, NA, NB, NC, ws, bf, myid, UA, UB, UC;
  double a1, a2, wdist, l, eps, tke, dtmp, af, Pk, S;
  double dp[NDIMN], dv[3], dW[NGRPP], adu[NDIMN*NDIMN], rBA[NDIMN], nr[NDIMN], pc0[NDIMN],
         au[NDIMN], ge[9], au2[U2DOF];
  #ifndef WALLFUNCTIONS
  int RA, RB, RC;
  double c0, var, fmean;
  ...
  #ifdef _OPENMP
    #ifndef WALLFUNCTIONS
      #pragma omp parallel private(myid,p,p2,p3,pNGRPP,a1,a2,eN,A,B,C,NA,NB,NC,UA,UB,UC,RA,RB,RC,au,adu,addu,af,tke,\
                                   eps,au2,Pk,S,dp,ge,c0,dW,dv,dtmp,j,jN,wdist,wdist0,rBA,nr,l,fmean,var,pc0)\
	  		           reduction(+:cn,ws,bf)
    #else
      #pragma omp parallel private(myid,p,p2,p3,pNGRPP,a1,a2,eN,A,B,C,NA,NB,NC,UA,UB,UC,au,adu,af,tke,\
                                   eps,au2,Pk,S,dp,ge,dW,dv,dtmp,j,jN,wdist,rBA,nr,l,pc0)\
			           reduction(+:ws,bf)
    #endif // WALLFUNCTIONS
  #endif // _OPENMP
  {
    ...
    for ( p = 0; p < npar; p++ )		// loop over all particles
    {
      ...
      #ifndef WALLFUNCTIONS
      addu[0] = (ddu[NA+0]+ddu[NB+0]+ddu[NC+0])/3.0;			// d^2<U>/dx^2 + d^2<U>/dy^2
      addu[1] = (ddu[NA+1]+ddu[NB+1]+ddu[NC+1])/3.0;			// d^2<V>/dx^2 + d^2<V>/dy^2
      #endif
      ...
      Pk = -au2[0]*adu[0] - au2[3]*(adu[1]+adu[2]) - au2[1]*adu[3];	// production of turbulent kinetic energy
      S = COM2 - COM1*Pk/eps;						// source/sink for turbulent frequency
        ...
        #endif
        for ( j = 0; j < 9; j++ )
          ge[j] = (rho[RA+j] + rho[RB+j] + rho[RC+j])/3.0;		// tensor rho (wiggly-p_ij)
        c0 = -2.0*(ge[0]*au2[0] + ge[4]*au2[1] + ge[8]*au2[2]		// c0 = -2rho_ij<uiuj>/(3k*eps)
                + (ge[1]+ge[3])*au2[3] + (ge[2]+ge[6])*au2[4] + (ge[5]+ge[7])*au2[5])/(3.0*tke*eps);
        ge[0] -= 0.5*eps; ge[4] -= 0.5*eps; ge[8] -= 0.5*eps;		// ge -= 0.5*eps*delta_ij
        #ifdef __INTEL_COMPILER
        #pragma vector always
        #endif
      ...
      dtmp = sqrt(c0*eps*dt);
      dv[0] = -dp[0]*dt + redt*addu[0] + sredt*(adu[0]*dW[0] + adu[1]*dW[1])*stage
              + (ge[0]*(parvel[p3+0]-au[0]) + ge[1]*(parvel[p3+1]-au[1]) + ge[2]*parvel[p3+2])*dt
              + dtmp*dW[2]*stage;
      dv[1] = -dp[1]*dt + redt*addu[1] + sredt*(adu[2]*dW[0] + adu[3]*dW[1])*stage
              + (ge[3]*(parvel[p3+0]-au[0]) + ge[4]*(parvel[p3+1]-au[1]) + ge[5]*parvel[p3+2])*dt
              + dtmp*dW[3]*stage;
      dv[2] = 0.0
              + (ge[6]*(parvel[p3+0]-au[0]) + ge[7]*(parvel[p3+1]-au[1]) + ge[8]*parvel[p3+2])*dt
              + dtmp*dW[4]*stage;
      ...
      dv[1] = -dp[1]*dt
              + (ge[3]*(parvel[p3+0]-au[0]) + ge[4]*(parvel[p3+1]-au[1]) + ge[5]*parvel[p3+2])*dt
              + dtmp*dW[1]*stage;
      dv[2] = (ge[6]*(parvel[p3+0]-au[0]) + ge[7]*(parvel[p3+1]-au[1]) + ge[8]*parvel[p3+2])*dt
              + dtmp*dW[2]*stage;
      #endif
      ...
      #ifndef WALLFUNCTIONS	// with elliptic relaxation
      parfreq[p] += (-C3*af*(parfreq[p]-af) - S*af*parfreq[p])*dt + af*sqrt(2.0*C3*C4*parfreq[p]*dt)*dW[5]*stage;
      #else			// with wall-functions
      parfreq[p] += (-C3*af*(parfreq[p]-af) - S*af*parfreq[p])*dt + af*sqrt(2.0*C3*C4*parfreq[p]*dt)*dW[3]*stage;
      #endif
      ...
      for ( j = 0; j < nwe; j++ )		// loop over all wall-elements
						...
						// (this is not so bulletproof, but faster than checking all)
        {
          ...
          wdist = (wrBA[jN+0]*(parcoord[p2+1]-wrA[jN+1]) - wrBA[jN+1]*(parcoord[p2+0]-wrA[jN+0]))/wel[j];

	  #ifndef WALLFUNCTIONS
          // compute distance between old particle position and wall-element j
          wdist0 = (wrBA[jN+0]*(pc0[1]-wrA[jN+1]) - wrBA[jN+1]*(pc0[0]-wrA[jN+0]))/wel[j];
	  #endif
	  ...
	  // test particle wall-trajectories
          if ( wdist < 0.0 )			// trajectory 1
          {
            ...
            nr[1] = wenr[jN+1];
            l = 2.0*(rBA[1]*(wrA[jN+0]-parcoord[p2+0]) + rBA[0]*(parcoord[p2+1]-wrA[jN+1])) /
	    ...
	    fmean = f[wA[j]];
	    var = C4*fmean*fmean;
            CheckVslError( vdRngGamma(GAMMA_METHOD, stream[myid], 1, parfreq+p, fmean*fmean/var, 0.0, var/fmean) );
	    #else
	    // specify particle conditions based on wall-functions
            parwallfunctionbc( p, tke, af, au, au2, dp, rBA, nr, parvel, parfreq );
	    #endif
      ...
      // put back particle if it tries to escape from the domain through illegal means
      // (this is a sanity check (and fix) in case in the above fancy wall-condition something went wrong,
      // or rather the particle nastily sneaked through between two wall elements)
      if ( (parcoord[p2+0]*parcoord[p2+0] + parcoord[p2+1]*parcoord[p2+1]) < D*D/4 )
      {
         parcoord[p2+0] = pc0[0];			// put it back where it was in the previous timestep
         parcoord[p2+1] = pc0[1];
      }
	...
	parc[p] = 0.0;			// no scalar coming in at inflow
      ...
      if (!PARINEL(p,elp[p],a1,a2,coord,inpoel,parcoord,Ae))
        findpar(p,elp[p],myid,nelem,minx,maxx,npeldt,elp,inpoel,esuel,esupel1,esupel2,coord,parcoord,Ae,&bf);
      ...
      if (t > RETIME)				// release scalar at RETIME
      {
        ...
        #ifndef WALLFUNCTIONS
        //partm[p] = fmin( Cs*pow(DSHS/eps,1.0/3.0) + Ct*tke/eps, fmax(tke/eps,CT/sqrt(RE*eps)) );
  ...
  #endif
  (*wallstruck) = ws;		// number of particles that struck the wall in this timestep (for diagnostics)
  (*bruteforced) = bf;		// number of particles that had to be brute-forced in this timestep (for diagnostics)
  ...
  improvepar2( nelem, npar, nthreads, impcnt, npeldt, elp, npel, parcoord, parvel, psel1, psel2
               , parfreq
               #ifdef MICROMIXING
	       , parc
	       #endif
...
//
// calculates the size of the next timestep
//
{
  int p, pN;
  double vel, dt, mindt;
...
void langevin( int nelem, int npoin, int nbpoin, int it0, int it, int nwe, int npar,
               int nthreads, int restarted, int maxn, int ec_size,
	       double minx, double miny, double maxx, double maxy, double t,
               double *drag, double *lift, double *tdrag, double *tlift,
	       int *wec,
	       int *esup1, int *esup2, int *bpg, int *inpoel, int *binpoel, int *npel,
	       int *elp, int *npeldt, int *we, int *weo, int *esuel, int *betags,
	       int *esupel1, int *esupel2, int *onum, int *az1, int *az2, double *r_az, double *dir_az,
               double *N_az,
	       double *odpn, double *oudt, double *wenr, double *sqrtAp,
	       double *wrBA, double *wrA, double *wel, double *prhs,
	       double *dpr, double *Ae, double *dNx, double *dNy, double *parcoord, double *parvel,
	       double *u, double *ue,
	       double *tu, double *tpr, double *du, double *ddu,
	       double *coord, double *pr, double *rg, sparsemat *P, int *psel1, int *psel2,
               int ndl, int *es1, int *es2, int *ec, double *uprof, double *vprof, double *u2prof, double *v2prof,
               double *u3prof, double *v3prof, double *u4prof, double *v4prof, double *uvprof,
               double *tkeprof, double *epsprof, double *dete, double *yprof, double *xprof_c, double *uprof_c,
               double *pprof_c, int angle_size, int *inprof_w, double *angle_w,
               double *parfreq, double *u2, double *u3, double *u4, double *f, double *u2e, double *u3e,
               double *u4e, double *fe, double *tu2, double *tu3, double *tu4, double *tf, double *tdu
	       #ifndef WALLFUNCTIONS
               , int *bpsup1, int *bpsup2, int *bptags, double *ru, VSLStreamStatePtr *stream, int *wA,
               int *wlc, double *wpnr, double *wnz, double *rrhs, sparsemat *R, double *rho
	       #endif
	       #ifdef MICROMIXING
	       , double *parc, double *partm, double *c, double *c2, double *c3, double *c4, double *tm,
	       double *uc, double *ce, double *c2e, double *c3e, double *c4e, double *tme, double *uce,
	       double *tc, double *tc2, double *tc3, double *tc4, double *ttm, double *tuc, int npl,
	       int *epdfloc, int *sl, double *tpdf, double *cprof,
...
//
// implements a main timestepping loop using the Langevin equations
//
{
  int wallstruck=0, pit=0, minnp=0, sm=10000, ne=0, lne=0, nc=0, impcnt, msc=0, nic=0, bf, stage;
  ...
  for ( ; (t < MAXTIME) && (it < MAXTS); t+=dt, it++ )
  {
    ...
    stage = 1;					// Euler-Maruyama timestepping scheme
      ...
      #ifndef WALLFUNCTIONS
      // elliptic relaxation
      rit = rstep( nelem, nbpoin, nthreads, bpg, inpoel, wlc, bpsup1, bpsup2, bptags, Ae, wpnr,
                   rrhs, rho, dNx, dNy, u2, f, wnz, du, R );
      #endif

      // advance particles
      lstep( stage, nelem, npar, nwe, nthreads, inpoel, &impcnt, elp, npeldt, wec, esuel, npel, esupel1,
             esupel2, dt, minx, miny, maxx, maxy, parcoord, parvel, dNx, dNy, wrBA, wrA, wel, wenr, du, Ae, pr, rg,
      ...
      if (stage == STAGES-1)	// pressure projection only in the last stage
      {
        ...
        pit = pstep( nwe, npoin, nelem, nbpoin, nthreads, dt, maxx, inpoel, binpoel, we, weo, bpg,
                     betags, odpn, oudt, wel, wenr, prhs, du, ddu, pr, dpr, u2, dNx, dNy, Ae, coord, P, u );
      ...
      ne = stat( nelem, npoin, nbpoin, npar, nthreads, dt, npel, esup1, esup2, inpoel, elp, nwe,
                 wenr, ue, u, du, ddu, parvel, dNx, dNy, &nc, betags, binpoel, oudt, u2e, u3e,
                 u4e, fe, u2, u3, u4, f, parfreq
	         #ifndef WALLFUNCTIONS
	         , bptags, bpg, we, weo
	         #endif
                 #ifdef MICROMIXING
      ...
      {
      #endif
        // save normal derivative of mean pressure at wall-elements for next timestep
        save_old_dpn( nwe, nbpoin, binpoel, betags, wenr, ddu, odpn, oudt, we, inpoel, u2, dNx, dNy, u, du );
      #ifdef ME
      }
      #endif
    ...
    if (!(it%ST2DUMP))					// dump results and restartfile at every ST2DUMPth iteration
    {
       ...
       // output restartfile
       #ifdef SAVERESTART  
       saverestartpar( nthreads, npoin, npar, *onum, it, t, parcoord, parvel, tu, tdu, parfreq, tu2, tu3, tu4, tf, tpr
                       #ifndef WALLFUNCTIONS
                       , stream
                       #endif
		       #ifdef MICROMIXING
    ...
    out_inst_dist( npoin, nbpoin, minnp, pit, maxn, t, dt, drag, lift, tdrag, tlift, betags, binpoel, onum, u,
                   wenr, wel, du, ndl, maxx, maxy, es1, es2, ec_size, ec, az1, az2,
		   xprof_c, uprof_c, pprof_c, inpoel, coord,
                   uprof, vprof, u2prof, v2prof, u3prof, v3prof, u4prof, v4prof, uvprof, tkeprof, epsprof, dete, yprof,
                   angle_size, inprof_w, angle_w, r_az, dir_az, N_az,
                   #ifndef WALLFUNCTIONS
		   rit,
		   #endif
		   #ifdef MICROMIXING
		   c, c2, c3, c4, tm, uc, tc, tc2, tc3, tc4, ttm, tuc, npl, epdfloc,
		   esupel1, esupel2, c2e, psel1, psel2, parc, ce, sl, tpdf,
                   cprof, c2prof, c3prof, c4prof, tmprof,
		   #endif
