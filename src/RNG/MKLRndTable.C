...
/*!
  ...
  \author    J. Bakosi
  \date      Sun 10 Nov 2013 06:18:32 AM MST
...
using tk::MKLRndTable;

MKLRndTable::MKLRndTable(int nthread,
                         int brng,
                         RndDist dist,
                         int method,
                         unsigned int seed,
  ...
  m_remainder(number % nthread)
{
  ...
  m_stream =
    std::unique_ptr<VSLStreamStatePtr[]>(new VSLStreamStatePtr [m_nthread]);
  ...
  m_rnd = std::unique_ptr<real[]>(new real [number]);
...
//******************************************************************************
{
  if (m_dist == RndDist::UNIFORM) {
    ...
    #endif
    for (int t=0; t<m_nthread; ++t) {
      uniform(m_method, m_stream[t], m_chunk, m_rnd.get() + t*m_chunk,
              UNIFORM_LEFT_BOUND, UNIFORM_RIGHT_BOUND);
    }
    uniform(m_method, m_stream[0], m_remainder, m_rnd.get() + m_nthread*m_chunk,
            UNIFORM_LEFT_BOUND, UNIFORM_RIGHT_BOUND);
    ...
    #endif
    for (int t=0; t<m_nthread; ++t) {
      gaussian(m_method, m_stream[t], m_chunk, m_rnd.get() + t*m_chunk,
               GAUSSIAN_MEAN, GAUSSIAN_STD);
    }
    gaussian(m_method, m_stream[0], m_remainder, m_rnd.get() + m_nthread*m_chunk,
             GAUSSIAN_MEAN, GAUSSIAN_STD);
    ...
    #endif
    for (int t=0; t<m_nthread; ++t) {
      gamma(m_method, m_stream[t], m_chunk, m_rnd.get() + t*m_chunk,
            GAMMA_SHAPE, GAMMA_DISPLACEMENT, GAMMA_SCALE);
    }
    gamma(m_method, m_stream[0], m_remainder, m_rnd.get() + m_nthread*m_chunk,
          GAMMA_SHAPE, GAMMA_DISPLACEMENT, GAMMA_SCALE);
