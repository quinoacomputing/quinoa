...
CompFlowProblemSedovBlastwave::initialize( ncomp_t,
...
// *****************************************************************************
{
  ...
  if (centering == tk::Centering::ELEM) {
    ...
    if (std::abs(x) < eps && std::abs(y) < eps && std::abs(z) < eps)
      p = g_newinputdeck.get< newtag::compflow, newtag::p0 >();
    else
      p = 0.67e-4;
...
tk::InitializeFn::result_type
CompFlowProblemSedovBlastwave::analyticSolution(
  ncomp_t,
  const std::vector< EOS >& mat_blk,
  tk::real x,
  tk::real y,
...
// *****************************************************************************
{
  return initialize( 0, mat_blk, x, y, z, 0 );
}
