      ...
      for (std::size_t e=0; e<nelem; ++e)
      {
        ...
        if (err > tol)
        {
          ...
          for (std::size_t k=0; k<nmat; ++k)
          {
            ...
            zk_sum += alpha[k]*(1-alpha[k])*zk[k];
            // Tipton
            // zk[k] = gamma*(pressure[k]+pinf); // Here zk is Kk the compressibility modulus
            // zk_sum += alpha[k]/zk[k];
          }
          ...
          for (std::size_t k=0; k<nmat; ++k)
          {
            ...
            // Favrie's
            x[2*nmat] += zk[k]*alpha[k]*(1-alpha[k])*pressure[k]/zk_sum;
            // // Tipton
            // x[2*nmat] += alpha[k]/zk[k]*pressure[k]/zk_sum;
          }
