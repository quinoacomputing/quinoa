                      ...
                      const std::vector< std::size_t >& ndofel ) const
    {
      ...
      for (std::size_t e=0; e<nelem; ++e)
      {
        ...
        if (pref) {
          ...
          // for P0P1, reconstruct all variables
          if (is_p0p1)
            for (std::size_t c=0; c<m_ncomp; ++c) vars.push_back(c);
          // for high-order DG, reconstruct only volume fractions
          else if (ndof > 1)
            for (std::size_t k=0; k<nmat; ++k) vars.push_back(volfracIdx(nmat, k));
        }
