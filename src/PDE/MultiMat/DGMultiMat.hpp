                   ...
                   tk::Fields& Pnode ) const
    {
      ...
      for (std::size_t e=0; e<nielem ; ++e)
      {
        ...
        for (std::size_t i=0; i<4; ++i)
        {
          ...
          auto pgp = tk::eval_state(nprim(), m_offset, rdof, rdof, e, P, B);

          //if ((nmat > 1) && (intcompr > 0))
          //  tk::THINCReco(m_system, m_offset, rdof, nmat, e, inpoel, coord,
          //    geoElem, {{xi, eta, zeta}}, U, P, ugp);

          for (std::size_t c=0; c<m_ncomp; ++c)
            Unode(inpoel[4*e+i], c, m_offset) += ugp[c];
          for (std::size_t c=0; c<nprim(); ++c)
            Pnode(inpoel[4*e+i], c, m_offset) += pgp[c];
        }
      }
    }
