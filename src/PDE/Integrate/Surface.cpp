...
// *****************************************************************************
{
  ...
  for (auto f=fd.Nbfac(); f<esuf.size()/2; ++f)
  {
    ...
    for (std::size_t igp=0; igp<ng; ++igp)
    {
      ...
      auto state_U_grad_l = eval_state_gradient (ncomp, ndof, rdof, 
                         el, U, dBdx_l );
      auto state_P_grad_l = eval_state_gradient (nprim, ndof, rdof, 
                         el, P, dBdx_l );
      for (ncomp_t c=0; c<ncomp; ++c){
          grad_all.push_back(state_U_grad_l[c]);
      }
      ...
      auto fl_vis_r = visc_flux(nprim, mat_blk, state[1], grad_all); 
