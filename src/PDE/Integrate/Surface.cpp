...
// *****************************************************************************
{
  ...
  for (auto f=fd.Nbfac(); f<esuf.size()/2; ++f)
  {
    ...
    for (std::size_t igp=0; igp<ng; ++igp)
    {
      ...
      B[0].resize(dof_el);
      B[1].resize(dof_er);
      eval_basis( dof_el, ref_gp_l[0], ref_gp_l[1], ref_gp_l[2], B[0] );
      eval_basis( dof_er, ref_gp_r[0], ref_gp_r[1], ref_gp_r[2], B[1] );
      ...
      // Gradients of basis functions
      for (std::size_t i=0; i<3; +i) {
        dBdx[0][i].assign( dof_el, 0.0 );
        dBdx[1][i].assign( dof_er, 0.0 );
      }

      auto jacInv_l =
        inverseJacobian( coordel_l[0], coordel_l[1], coordel_l[2], coordel_l[3] );
      eval_dBdx_p1( dof_el, jacInv_l, dBdx[0] );
      auto jacInv_r =
        inverseJacobian( coordel_r[0], coordel_r[1], coordel_r[2], coordel_r[3] );
      eval_dBdx_p1( dof_er, jacInv_r, dBdx[1] );

      // Compute high-order face states (reconstruction)
      state[0] = viscousRhs.stateAt( mat_blk, U, P, el, dof_el, B[0] );
      state[1] = viscousRhs.stateAt( mat_blk, U, P, er, dof_er, B[1] );
      ...
      // Compute viscous fluxes
      viscousRhs.interiorFlux( mat_blk, ncomp, state, cellAvgState, fn,
        centroids, grad, fl ); // currently no-op
