              ...
              tk::Fields& R ) const
    {
      ...
      if (!pref) {
        ...
        tk::surfInt_constP( 1, m_mat_blk, t, ndof, rdof, inpoel, solidx,
          ...
          dt, R, riemannDeriv, viscous);

        // compute boundary surface flux integrals
        for (const auto& b : m_bc)
          tk::bndSurfInt_constP( 1, m_mat_blk, ndof, rdof, std::get<0>(b), fd,
            geoFace, geoElem, inpoel, coord, t, m_riemann, visc_flux, velfn,
            std::get<1>(b), U, P, R, riemannDeriv, viscous );

        // compute volume integrals
        tk::volInt_constP( 1, t, m_mat_blk, ndof, rdof, nelem, inpoel, coord,
          geoElem, flux, visc_flux, velfn, Problem::src, U, P, R, viscous );
      }
      else {
        // compute internal surface flux integrals
