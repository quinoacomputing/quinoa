  ...
  for (const auto& s : bcconfig) {       // for all bc sidesets
    ...
    if (bc != end(bface))
    {
      ...
      for (const auto& f : bc->second)
      {
        ...
        auto ul = eval_state( ncomp, offset, rdof, 1, el, U, B, {0, ncomp} );
        auto uprim = eval_state( nprim, offset, rdof, 1, el, P, B, {0, nprim} );
...
// *****************************************************************************
{
  ...
  state = eval_state( ncomp, offset, rdof, dof_e, e, U, B, {0, ncomp} );
  sprim = eval_state( nprim, offset, rdof, dof_e, e, P, B, {0, nprim} );
