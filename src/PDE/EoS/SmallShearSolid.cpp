...
// *************************************************************************
{
  ...
  for (std::size_t i=0; i<3; ++i) {
    for (std::size_t j=0; j<3; ++j)
      asig[i][j] += std::max(0.0, (1.0-damage)) * m_mu*alpha*devbt[i][j];
  }
...
// *************************************************************************
{
  ...
  tk::real a = std::sqrt(al_eff * std::max(0.0, (1.0-damage)) * m_mu/arho);
...
// *************************************************************************
{
  ...
  auto rhoEe = std::max(0.0, (1.0-damage)) * m_mu * eps2;
