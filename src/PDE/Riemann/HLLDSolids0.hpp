...
struct HLLDSolids {
        ...
        const std::vector< std::array< tk::real, 3 > >& )
  {
    ...
    // Signal velocities
    auto Sl_minus = std::min((vnl-ac_l), (vnr-ac_r));
    auto Sl_plus = std::max((vnl+ac_l), (vnr+ac_r));
    auto Si = (rhol*vnl*(Sl_minus-vnl) - rhor*vnr*(Sl_plus-vnr) + signn_l-signn_r)
      (rhol*(Sl-vnl) - rhor*(Sr-vnr));
    
    // Compute tilde states
    for (std::size_t k=0; k<nmat; ++k)
    {
      // Left state
      // -----------------------------------------------------------------------
