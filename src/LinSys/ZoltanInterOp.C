...
/*!
  ...
  \author    J. Bakosi
  \date      Mon 16 Nov 2015 03:43:10 PM MST
...
class InciterMeshElemAdapter : public Zoltan2::MeshAdapter< User > {
  ...
  public:
    ...
    //! Constructor
    //! \param[in] graph Mesh graph
    //! \param[in] nelem Total number of elements in mesh graph
    InciterMeshElemAdapter(
    ...
    : m_graph( graph ),
      ...
      m_elemcoord( elemcoord ),
      m_elemid( elemid )
    {}

    //! Returns the global number of mesh entities
    //! \return Global (aggregate) number of mesh elements
    ...
    { return m_graph.tetinpoel().size()/4; }
    ...
    {
      coords = m_elemcoord[ static_cast<std::size_t>(dim) ].data();
      stride = 1;
    }

  private:
    const tk::UnsMesh& m_graph;          //!< Unstructured mesh reference
    ...
    //! Mesh element coordinates
    const std::array< std::vector< tk::real >, 3 >& m_elemcoord;
...
//! \param[in] graph Unstructured mesh graph object reference
//! \param[in] elemcoord Mesh element coordinates
...
//******************************************************************************
{
  ...
  using InciterZoltanAdapter = InciterMeshElemAdapter< ZoltanTypes >;
  InciterZoltanAdapter adapter( graph, nelem, elemcoord, elemid );
  ...
  //const part_t *partAssignments =
  auto partlist = partitioner.getSolution().getPartListView();
  std::vector< std::size_t > chare;
  for (int p=0; p<graph.tetinpoel().size()/4; ++p )
    chare.push_back( static_cast< std::size_t >( partlist[p] ) );
...
// 
//   // Perform partitioning using Zoltan
//   if (peid == 0) print.diagstart( "Partitioning mesh ..." );
