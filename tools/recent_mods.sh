...
export mods=$(paste -d "\n" <(git ls-tree -r --name-only HEAD $1/doc | while read filename; do echo "$(git log -1 --format="%ai" -- $filename) $filename"; done | sort -r | grep dox$ | head -n 5 | awk '{print $4}' | xargs grep -i -- '@page\|\\page\|@mainpage\|\\mainpage' | awk '{ if (!length($3)) print "  @ref index"; else print "  @ref " $3 }') <(git ls-tree -r --name-only HEAD $1/doc | while read filename; do echo "$(git log -1 --format="%ai" -- $filename) $filename"; done | sort -r | grep dox$ | head -n 5 | awk '{print $1}' | xargs -i date -d "{}" +"%b %d, %Y" | awk '{print "  @m_span{m-text m-dim} <em> " $0 " </em> @m_endspan <br>" }'))
