// *****************************************************************************
/*!
  \file      tests/unit/Inciter/EOS/TestJWL.cpp
  \copyright 2012-2015 J. Bakosi,
             2016-2018 Los Alamos National Security, LLC.,
             2019-2021 Triad National Security, LLC.
             All rights reserved. See the LICENSE file for details.
  \brief     Unit tests for PDE/EoS/JWL.hpp
  \details   Unit tests for PDE/EoS/JWL.hpp
*/
// *****************************************************************************
...
using JWL_group = test_group< JWL_common, MAX_TESTS_IN_GROUP >;
using JWL_object = JWL_group::object;

//! Define test group
static JWL_group JWL( "PDE/EoS/JWL" );

//! Charm++ chare to test Pack/Unpack utilities during network migration
class Receiver : public CBase_Receiver {
  public:
    //! Constructor taking (and migrating) a JWL
    explicit Receiver( const inciter::EOS& s,
                       int expected,
                       const std::string& label )
    {
      // Create test result struct, assume test is ok
      tut::test_result tr( "PDE/EoS/JWL", 1,
                           "Charm:migrate EOS(" + label + ") 2",
                           tut::test_result::result_type::ok );

      try {
        auto actual = s.index();
        // Evaluate test
        ensure_equals( "EOS underlying type different after migrated",
                       actual, expected );
